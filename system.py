...
import torch
def get_approximated_function(Q, R, p_history, w_history, A, B, K, H):
...
def eta(W, T, t, initial=1):
    initial = 1
    # 簡易的に定数は無視して1からスタート
    return initial/(W * np.sqrt(t+1))# /(min(1 + 0.01*t, 10))
    # * 2**(int(10*t/T)))
...
def adaptive_control(CONSTANTS):
    ...
    for t in range(T):
        if t <= 2 * H:
            ...
            grad = get_approximated_function(Q, R, p_data_update.flip(0), w_data_update.flip(0), A, B, K, H)
            if p_data[t] != 0:
