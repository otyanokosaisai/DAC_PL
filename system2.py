...
def generate_packetloss_in_advance(T, f_G, f_L, H):
    """
    ...
    for i in tqdm.tqdm(range(sequence_num)):
        if np.random.rand() < f_G:
            loss_length = max(min(int(H * f_L), np.random.poisson(H * f_L / 2)), 1)
...
def evaluate_system(CONSTANTS, A, B, K):
    ...
    print("Eigenvalues of A - BK:", eigen_values)
    print("A 2norm:", np.linalg.norm(A, 2))
    print("A - BK 2norm:", np.linalg.norm(A - B @ K, 2))
    print("A - BK ** 10 norm:", np.linalg.norm(np.linalg.matrix_power(A - B @ K, 10), 2))
    print("K 2norm:", np.linalg.norm(K, 2))
    gamma = max(np.abs(eigen_values))
    kappa_A = np.linalg.norm(A, 2)

    CONSTANTS["kappa_A"] = kappa_A
    CONSTANTS["kappa_ABK"] = np.linalg.norm(A - B @ K, 2)
    CONSTANTS["kappa_K"] = max(np.linalg.norm(K, 2), np.linalg.norm(eigen_vectors, 2), np.linalg.norm(np.linalg.inv(eigen_vectors), 2))
...
import torch
def get_approximated_function(Q, R, p_history, w_history, A, B, K, H, M):
...
def update_M(M, grad, lr, norm_bound, nu):
    ...
    for i in range(M.shape[0]):
        U, S, V = torch.svd(M_tmp[i])
        S = torch.clamp(S, max=norm_bound*nu**i)
        M_tmp[i] = U @ torch.diag(S) @ V.T
    return M_tmp.clone().detach().requires_grad_(True)
...
def adaptive_control(CONSTANTS):
    ...
    for t in range(T):
        if t <= 2 * H:
            ...
            p_data_update = torch.cat([torch.zeros(2*H+1 - p_data_update.shape[0]), p_data_update], dim=0)
            ...
            grad = get_approximated_function(Q, R, p_data_update.flip(0), w_data_update.flip(0), A, B, K, H, M)
            if p_data[t] != 0:
                M = update_M(M, grad, eta(W, T, t, initial_eta), 
                            kappa_B * 2 * np.linalg.norm(K, 2)**3, 
                            nu)
            ...
            grad = get_approximated_function(Q, R, 
                # p_data[t-H-1:t].flip(0), w_data[t-2*H:t+1].reshape(2*H+1, 2, 1).flip(0), 
                p_data_update.flip(0), w_data_update.flip(0),
                A, B, K, H, M)
            if p_data[t] != 0:
                M = update_M(M, grad, eta(W, T, t, initial_eta), 
                         kappa_B * 2 * np.linalg.norm(K, 2)**3, 
        ...
        # TensorBoardにログを記録
        writer.add_scalar("Regret/Instant", reg, t)
        ...
        if t % 10 == 0:
            writer.add_scalar("Regret/Cumulative", sum(regret_history), t)

    # 最終的な結果を保存
    writer.close()

    # CSVにRegretを保存（オプション）
    np.savetxt(f"{CONSTANTS['save_dir']}/regret_history.csv", regret_history)
...
def search_optimal_linear_policy(CONSTANTS):
    ...
    B = CONSTANTS["B"]
    K = CONSTANTS["K"]
    # PL_data = CONSTANTS["PL_data"]
    # pertubations = CONSTANTS["pertubations"]
    T_max = 40000 #に絞って探索

    def objective(trial):
        # import os
        # print(f"Trial {trial.number} running on PID {os.getpid()}")
        ...
        if np.max(np.abs(L)) >= 1 or K1**2 + K2**2 > CONSTANTS["kappa"]**2:
            time.sleep(0.01)
            return 1e100

        # cost_history, _, _ = calculate_total_cost(T, A, B, PL_data, pertubations, K, np.array([0.0, 0.0]), Q_info, R_info, cycle_time, use_history=False)
    ...
    print("Minimum average cost:", study.best_value)

    # Eigenvalues of the optimal closed-loop system
    L, _ = scipy.linalg.eig(A - B @ best_K)
    print("Eigenvalues of A - BK:", L)

    # H = int(-np.log(T)/np.log(CONSTANTS["nu"]))
    CONSTANTS["K_opt"] = best_K

def searcher(W, T, f_G, n_l, l_L, eta_init, name="trial"):
    # W = 3
    # fg = 1
    # T = 10000
    # n_l = 1
    ...
    if n_l >= l_L:
        raise ValueError("n_l must be smaller than l_L")
    ...
    if True:
        # 一度に生成する長さ
        partial_length = 100000  # 必要に応じて調整
        pertubations = []

        # 部分的に生成してリストに追加
        for _ in range((T // partial_length) + 1):
            for _ in range(partial_length):
                pertubations.append(generate_perturbation_in_advance(np.array([0, 0]), W))

        # 必要な長さだけリピート
        pertubations = np.array(pertubations[:partial_length])  # まず最初の部分だけ
        repete_num = T // len(pertubations)
        remain_num = T % len(pertubations)
        
        # T に必要な長さまで繰り返す
        pertubations = np.tile(pertubations, (repete_num, 1))
        pertubations = np.concatenate([pertubations, pertubations[:remain_num]], axis=0)
    ...
    K_opt = np.array([[1.92914617, 1.58251583]])
    CONSTANTS["K_opt"] = K_opt
    print("Optimal policy found")

    regret_history = adaptive_control(CONSTANTS)
...
if __name__ == "__main__":
    ...
    # 0 1 2 3
    # 1 2 4 8
    name = "trial_unstable0_10"
    f_G = 1/(np.sqrt(T))
    n_L = 5
    l_L = 2*int(np.log(T))
    eta_init = 1
    searcher(W, T, f_G, n_L, l_L, eta_init, name)
